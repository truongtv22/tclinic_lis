...
export default {
  ...
  queryAll(
    ...
    const stmTotal = db.prepare(
      `SELECT COUNT(*) total FROM [dbo.KQ_SysmexXP100] ${whereClause}`,
    );
    const stmList = db.prepare(
      `SELECT * FROM [dbo.KQ_SysmexXP100] ${whereClause} ORDER BY date_time DESC`,
    );
  ...
  create(values: any = {}) {
    ...
    const stmAdd = db.prepare(
      `INSERT INTO [dbo.KQ_SysmexXP100] (
          ${Object.keys(values)
    ...
    const stmQueryById = db.prepare(
      `SELECT * FROM [dbo.KQ_SysmexXP100] WHERE id = @id`,
    );
  ...
  delete(id: number) {
    ...
    const stmDelete = db.prepare(
      `DELETE FROM [dbo.KQ_SysmexXP100] WHERE id = @id`,
    );
