...
import { Row, Col, Button, Card, Form, Radio, Select, Space, Input, InputNumber, Typography, AutoComplete, Popconfirm } from "antd";
      ...
      <Split lineBar className="space-x-2">
          ...
          <div className="space-y-2">
            {range(10).map((index) => (
              <div key={index} className="flex space-x-2">
                {devices[index] ? <StarFilled className="text-yellow-400" /> : <StarOutlined />}
                <Radio
                  value={1}
                ...
                >
                  <span className="line-clamp-2">{devices[index]?.name || "..."}</span>
                </Radio>
              </div>
            ))}
          </div>
        ...
        <Card className="flex-1 rounded" size="small">
          <Form form={form} layout="vertical" onFinish={onSubmit}>
            <Row gutter={8} align="middle" justify="space-between" className="mb-2">
              <Col>
              ...
              <Row gutter={4}>
                {devices[selected] && (
                  <Col>
                    <Popconfirm title="Xoá thiết bị" description="Bạn muốn xóa thiết bị này không?" okText="Xoá" cancelText="Không" onConfirm={onDelete}>
            ...
            <Row gutter={8}>
              <Col sm={24} md={12}>
                <Form.Item name="path" label="ComPort" initialValue="COM1">
                  <AutoComplete
                    options={["COM1", "COM2", "COM3", "COM4", "COM5", "COM6", "COM7", "COM8", "COM9", "COM10"].map((v) => ({
                      value: v,
                      label: v,
                    }))}
                  />
              ...
              <Col sm={24} md={12}>
                <Form.Item name="baudRate" label="BaudRate" initialValue={9600}>
                  <AutoComplete options={[110, 300, 1200, 2400, 4800, 9600, 14400, 19200, 38400, 57600, 115200].map((v) => ({ value: v, label: v }))} />
                </Form.Item>
              </Col>
            </Row>
            <Row gutter={8}>
              <Col sm={24} md={12}>
                <Form.Item name="dataBits" label="DataBits" initialValue={8}>
                  <Select options={[5, 6, 7, 8].map((v) => ({ value: v, label: v }))} />
                </Form.Item>
              </Col>
              <Col sm={24} md={12}>
                <Form.Item name="stopBits" label="StopBits" initialValue={1}>
                  <Select options={[1, 1.5, 2].map((v) => ({ value: v, label: v }))} />
                </Form.Item>
              </Col>
            </Row>
            <Row gutter={8}>
